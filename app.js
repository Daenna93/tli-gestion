...
TLI.Firebase = {
  ...
  mergeData: function(remoteData) {
    // Fusionner les orders (par ID, fusionner les photos)
    if (remoteData.orders) {
      var orderMap = {};
      TLI.Data.orders.forEach(function(o) { orderMap[o.id] = o; });
      remoteData.orders.forEach(function(ro) {
        if (!orderMap[ro.id]) {
          orderMap[ro.id] = ro;
        } else {
          var lo = orderMap[ro.id];
          // Fusionner les photos (dédoublonnage par src)
          if (ro.photos) {
            if (!lo.photos) lo.photos = [];
            var existingPhotos = {};
            lo.photos.forEach(function(p) { existingPhotos[p] = true; });
            ro.photos.forEach(function(p) {
              if (!existingPhotos[p]) lo.photos.push(p);
            });
          }
          // Mettre à jour les autres champs
          ['name','service','type','machineId','material','weight','time','price',
           'priority','notes','status','dateFinished','dateDelivered'].forEach(function(f) {
            if (ro[f] !== undefined) lo[f] = ro[f];
          });
        }
      });
      TLI.Data.orders = Object.values(orderMap);
    }

    // Fusionner les clients
    if (remoteData.clients) {
      var clientMap = {};
      TLI.Data.clients.forEach(function(c) { clientMap[c.id] = c; });
      remoteData.clients.forEach(function(rc) {
        if (!clientMap[rc.id]) {
          clientMap[rc.id] = rc;
        } else {
          var lc = clientMap[rc.id];
          ['name','company','role','phone','email','address','notes'].forEach(function(f) {
            if (rc[f] !== undefined) lc[f] = rc[f];
          });
        }
      });
      TLI.Data.clients = Object.values(clientMap);
    }

    // Fusionner les models
    if (remoteData.models) {
      var modelMap = {};
      TLI.Data.models.forEach(function(m) { modelMap[m.id] = m; });
      remoteData.models.forEach(function(rm) {
        if (!modelMap[rm.id]) {
          modelMap[rm.id] = rm;
        } else {
          var lm = modelMap[rm.id];
          ['name','category','type','time','weight','price','description'].forEach(function(f) {
            if (rm[f] !== undefined) lm[f] = rm[f];
          });
        }
      });
      TLI.Data.models = Object.values(modelMap);
    }

    // Fusionner le stock
    if (remoteData.stock) {
      var stockMap = {};
      TLI.Data.stock.forEach(function(s) { stockMap[s.id] = s; });
      remoteData.stock.forEach(function(rs) {
        if (!stockMap[rs.id]) {
          stockMap[rs.id] = rs;
        } else {
          var ls = stockMap[rs.id];
          ['type','name','qty','unit','alert','supplier'].forEach(function(f) {
            if (rs[f] !== undefined) ls[f] = rs[f];
          });
        }
      });
      TLI.Data.stock = Object.values(stockMap);
    }

    // Fusionner les événements de planning
    if (remoteData.planningEvents) {
      var eventMap = {};
      TLI.Data.planningEvents.forEach(function(e) { eventMap[e.id] = e; });
      remoteData.planningEvents.forEach(function(re) {
        if (!eventMap[re.id]) {
          eventMap[re.id] = re;
        } else {
          var le = eventMap[re.id];
          ['title','type','date','startTime','endTime','assignee','priority',
           'notes','clientId','clientName','reminder','reminderMinutes'].forEach(function(f) {
            if (re[f] !== undefined) le[f] = re[f];
          });
        }
      });
      TLI.Data.planningEvents = Object.values(eventMap);
    }

    // Fusionner les machines
    if (remoteData.machines) {
      var machineMap = {};
      TLI.Data.machines.forEach(function(m) { machineMap[m.id] = m; });
      remoteData.machines.forEach(function(rm) {
        if (!machineMap[rm.id]) {
          machineMap[rm.id] = rm;
        } else {
          var lm = machineMap[rm.id];
          if (rm.totalHours !== undefined) lm.totalHours = rm.totalHours;
          if (rm.maintenance) {
            if (!lm.maintenance) lm.maintenance = [];
            var maintMap = {};
            lm.maintenance.forEach(function(m) { maintMap[m.id] = m; });
            rm.maintenance.forEach(function(rmt) {
              if (!maintMap[rmt.id]) {
                maintMap[rmt.id] = rmt;
                lm.maintenance.push(rmt);
              }
            });
          }
        }
      });
      TLI.Data.machines = Object.values(machineMap);
    }

    // Fusionner les photos générales (par ID)
    if (remoteData.generalPhotos) {
      var photoMap = {};
      TLI.Data.generalPhotos.forEach(function(p) { photoMap[p.id] = p; });
      remoteData.generalPhotos.forEach(function(rp) {
        if (!photoMap[rp.id]) {
          photoMap[rp.id] = rp;
        }
      });
      TLI.Data.generalPhotos = Object.values(photoMap);
    }

    // Fusionner les qualityResults
    if (remoteData.qualityResults) {
      Object.keys(remoteData.qualityResults).forEach(function(key) {
        if (!TLI.Data.qualityResults[key]) {
          TLI.Data.qualityResults[key] = remoteData.qualityResults[key];
        }
      });
    }
  },
...
TLI.UI = {
  ...
  renderPhotos: function() {
    ...
    TLI.Data.generalPhotos.forEach(function(gp) {
      allPhotos.push({ src: gp.src, orderId: null, orderName: 'Photo generale', clientName: '', date: gp.date, id: gp.id });
    });
    TLI.Data.orders.forEach(function(o) {
      if (o.photos && o.photos.length > 0) {
        o.photos.forEach(function(p, idx) {
          allPhotos.push({ src: p, orderId: o.id, orderName: o.name, clientName: o.clientName, photoIdx: idx });
        });
      }
    });
    ...
    for (var i = 0; i < allPhotos.length; i++) {
      ...
      var item = document.createElement('div');
      item.className = 'photo-item';
      item.innerHTML = '<img src="' + TLI.Utils.escapeHtml(p.src) + '" alt="Photo"><button class="photo-delete">×</button>';
      (function(photoData) {
        item.querySelector('img').addEventListener('click', function() { TLI.UI.openGalleryPhoto(photoData); });
        item.querySelector('.photo-delete').addEventListener('click', function(e) {
          e.stopPropagation();
          TLI.UI.deleteGalleryPhoto(photoData.orderId, photoData.id, photoData.photoIdx);
        });
      })(p);
  ...
  deleteGalleryPhoto: function(orderId, photoId, photoIdx) {
    if (!confirm('Supprimer cette photo ?')) return;
    if (orderId) {
      TLI.Data.deletePhotoFromOrder(orderId, photoIdx);
    } else if (photoId) {
      TLI.Data.generalPhotos = TLI.Data.generalPhotos.filter(function(p) { return p.id !== photoId; });
      TLI.Data.save();
    }
    TLI.UI.showToast('Photo supprimée', 'warning');
    TLI.UI.renderPhotos();
    TLI.Firebase.push();
  },
...
TLI.Timer = {
  ...
  loop: function() {
    ...
    setInterval(function() {
      ...
      // Mettre à jour les timers dans la section Timer
      for (var i = 0; i < TLI.Data.activeTimers.length; i++) {
        var t = TLI.Data.activeTimers[i];
        var remaining = Math.max(0, t.endTime - Date.now());
        var pct = t.duration > 0 ? Math.min(100, ((t.duration - remaining) / t.duration) * 100) : 100;
      ...
      // Mettre à jour les timers sur le dashboard
      var dashList = document.getElementById('activeTimersList');
      var dashCard = document.getElementById('activeTimersCard');
      if (dashList && TLI.Data.activeTimers.length > 0) {
        dashList.innerHTML = '';
        for (var i = 0; i < TLI.Data.activeTimers.length; i++) {
          var t = TLI.Data.activeTimers[i];
          var remaining = Math.max(0, t.endTime - Date.now());
          var card = document.createElement('div');
          card.className = 'timer-card';
          card.innerHTML = '<div class="timer-name">' + TLI.Utils.escapeHtml(t.name) + '</div><div class="timer-info">🖨️ ' + TLI.Utils.escapeHtml(t.machineName) + '</div><div class="timer-remaining">' + TLI.Utils.formatTimer(remaining) + '</div>';
